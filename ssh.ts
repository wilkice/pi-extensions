 ...
 *   pi -e ./ssh.ts --ssh dev-server
 *   pi -e ./ssh.ts --ssh dev-server:/remote/path
 *
 * SSH destinations are passed directly to OpenSSH, so Host aliases from
 * ~/.ssh/config (including Include, ProxyJump, IdentityFile, etc.) work without
 * parsing the config in this extension.
 *
 * Requirements:
